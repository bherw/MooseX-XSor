...
sub xs_clone_instance {
	my ($self, $instance, $instance_slots, $clone, $clone_slots) = @_;

	#<<<
	return (
		$self->xs_create_instance($clone, $clone_slots, 'class_name'),
		map { (
			'{',
			'SV* val = ' . $self->xs_get_slot_value($instance_slots, $_) . ';',
			'if (val != &PL_sv_undef) {',
				$self->xs_set_slot_value($clone_slots, $_, 'val'),
				'if (SvWEAKREF(val)) {',
					$self->xs_weaken_slot_value($clone_slots, $_),
				'}',
			'}',
			'}'
		) } sort $self->get_all_slots,
	);
	#>>>
}

sub xs_initialize_all_slots {
	my ($self, $instance_slots) = @_;
	map { $self->xs_initialize_slot($instance_slots, $_) } sort $self->get_all_slots;
}
...
sub xs_slot_value_is_weak {
	my ($self, $instance_slots, $slot_name) = @_;
	'SvWEAKREF(' . $self->xs_get_slot_value($instance_slots, $slot_name) . ')';
}
...
=head2 C<xs_initialize_all_slots>

Generates statements which initialize all slots at once.

=head2 C<xs_initialize_slot>

	my @code = $mi->xs_initialize_slot($instance_slots, $slot_name);

Generates statements which initialize a slot.
...
=head2 C<xs_slot_value_is_weak>

	bool weak = @{[ $mi->xs_slot_value_is_weak($instance_slots, $slot_name) ]};

Generates an expression which returns true if the slot value is a weak ref.
