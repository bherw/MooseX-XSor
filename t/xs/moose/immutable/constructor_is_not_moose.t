...

{
	...
	::stderr_is(sub { Bar->meta->make_immutable(replace_constructor => 1) },
		q{}, 'no warning when replace_constructor is true');
}

is(Bar->meta->find_method_by_name('new')->package_name,
	'Bar', 'Bar->new is inlined, and not inherited from NotMoose');
...

{
	...
	::stderr_is(sub { Quux->meta->make_immutable },
...

{
	...
	::stderr_is(sub { Subclass->meta->make_immutable },
		q{}, 'no warning when inheriting from a class that has already made itself immutable');
}
