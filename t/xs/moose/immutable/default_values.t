...

{
	...
	my $e = ::exception { __PACKAGE__->meta->make_immutable };
	::is($e, undef,
...

{
	...
	{
		my $bar = Bar->new;
		::ok(!$bar->meta->get_attribute($_)->has_value($bar), "Attribute $_ has no value")
...

	{
		my $bar = Bar->new;
		::ok(
			!$bar->meta->get_attribute($_)->has_value($bar),
...
my $bar = Bar->new;
is($bar->foo, q{'},    'default value for foo attr');
is($bar->bar, q{\\},   'default value for bar attr');
is($bar->baz, q{"},    'default value for baz attr');
is($bar->buz, q{"'\\}, 'default value for buz attr');
is($bar->faz, qq{\0},  'default value for faz attr');
